...
class TestCases(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Examen
        list1 = expand(Path(10), self.map)
        for elem in list1:
            print(elem.route)
        list2 = depth_first_search(6, 1, self.map)
        print(list2.route)
        list3 = breadth_first_search(21, 16, self.map)
        print(list3.route)
        list4 = Astar([111, 194], [88, 69], self.map, 1)
        print(list4.route)
        list5 = Astar([88, 69], [44, 139], self.map, 3)
        print(list5.route)
        aux_list = [Path([3, 4, 5, 6]), Path([4, 5]), Path([5, 6])]
        list6 = calculate_cost(aux_list, self.map, 1)
        print(list6[0].g)
        aux_list = [Path([3]), Path([12, 8, 9]), Path([12, 8, 13])]
        list7 = calculate_heuristics(aux_list, self.map, destination_id=8, type_preference=1)
        print(list7[0].h)
        list8 = Astar([222, 101], [175, 98], self.map, 0)
        print(list8.g)
